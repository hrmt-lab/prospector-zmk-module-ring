...
**切替トリガ**
- Main ↔ AI Usage は **切替キー長押し**（既定 F21 = keycode `112`、約 700ms）または **ディスプレイダブルタップ**（CST816S）。
- キー長押しの判定は「押し続けて閾値に達した時点」で切替（離した瞬間ではない）。閾値 `RING_AI_USAGE_LONGPRESS_MS = 700`。
...
- MOD チップ（CTRL/SHFT/ALT/GUI）中心 `136 / 166 / 196 / 226`（間隔 30）、IME チップ中心 `256`。いずれも **縦中心 43**（レイヤー名の字面中心に整合）。
  - チップのグリフは `CONFIG_PROSPECTOR_MODIFIER_OS_*`（ビルド時固定）で切替。既定 Generic は `C/S/A/G`、Windows は `C/S/A/W`、Mac は `⌃/⇧/⌥/⌘`（Mac のみ `Symbols_Bold_26` を使用）。座標・縦中心は OS によらず共通。
...
- バー：幅 `52`、上端 `y=94`、最大高 `84`、角丸 `5`。x = `15 / 81 / 147 / 213`（中心 `41 / 107 / 173 / 239`）。4本の群（span 15–265, 幅250）を 280px 画面の中央（中心 140）に配置。リセット行確保のため最大高は 96→84 に縮小（実機で重なる場合は更に調整）。
...
- ツール名：Claude は左2本（中心 41/107）の中央 `cx=74`、Codex は右2本（中心 173/239）の中央 `cx=206`、`y=218`。
- プロバイダ間の縦区切り線：`x=140`（画面中央）、`y=78`〜`218`。
