...
- バー：幅 `52`、上端 `y=94`、最大高 `96`、角丸 `5`。x = `24 / 90 / 156 / 222`（中心 `50 / 116 / 182 / 248`）。
- 充填：`h = round(used% × 96 / 100)`、`fill_y = 94 + (96 − h)`（1px 下限）。色は固定ブランドカラー（Claude `#CC7E5A` / Codex `#7B6FE8`）。数値・ツール名も同色。
- 使用率数値：`y=194`（各バー中心 x）。ツール名：Claude `cx=72` / Codex `cx=214`、`y=212`。
- プロバイダ間の縦区切り線：`x=149`、`y=78`〜`212`。
