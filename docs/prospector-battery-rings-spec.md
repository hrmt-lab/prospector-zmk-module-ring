...
| 凡例ラベル（D） | UI Sans-serif | 9 | regular | |

### 4.2 LVGL での実装

MOD/STATE チップのアイコン文字は Unicode 記号とひらがなを使用。LVGLでは以下が必要：

```conf
CONFIG_LV_FONT_MONTSERRAT_8=y    # ラベル小
CONFIG_LV_FONT_MONTSERRAT_12=y   # チップ内・数値
CONFIG_LV_FONT_MONTSERRAT_14=y   # チップ内（余裕分）
```

レイヤー名のセリフ書体は **Cormorant Garamond** に確定（SIL OFL 1.1 ライセンス）。`lv_font_conv` で TTF を C 配列に変換して組み込む。

- サイズ：36px（2ペリフェラル以下）、30px（3ペリフェラル）の2サイズを用意
- 必要グリフ：ASCII + 基本記号のみ（日本語レイヤー名を使わない場合）
- ファイル名例：`font_cormorant_36.c`、`font_cormorant_30.c`

MOD/STATE チップ内の `⇪`（U+21EA）と `あ`（U+3042）は、チップ内テキストに使用するフォントに当該グリフを含める必要がある。Montserrat は `⇪` を含まないため、NotoSans 等 Unicode 記号対応フォントを別途用意するか、`⇪` の代替として SVG パスで直接描画する方法も検討すること。
...
セクションラベルなし（アイコンのみで識別）。

2×2グリッド配置。各チップは **circle r=12**。
...
- グリッドの上端 Y = 46（ヘッダから10px の余白）
- チップ半径 r = 12、間隔 = 6px（step = r×2 + gap = 30px）
- **ON状態**：fill=accent (`#E89B5C`)、文字 ui-sans 13 medium、color=text.primary
- **OFF状態**：fill=none, stroke=`#D0D5D8` 1px、文字 ui-sans 13 regular、color=text.tertiary

### 5.5 STATE（右パネル中段）

セクションラベルなし（アイコンのみで識別）。

1行2個配置。各チップは **circle r=10**。
...
- STATE 上端 Y = MOD下端 + msGap = (88+12) + 14 = 114、stCy = 114+10 = 124
- チップ半径 r = 10、STATEチップ間隔 = 10px（c2x = 218 + r×2 + 10 = 248）
- **MOD→STATE 間隔** = 14px
- ON/OFF の色規則は MODIFIERS と同様
- `⇪` は font-size 13、`あ` は font-size 11

### 5.6 OUT（右パネル下段）

STATEの下に区切り線（y=142）を引き、その下に配置。

```
sep line: x1=206 y1=142 x2=270 y2=142, stroke=#E2E5E8, stroke-width=0.5
```

| 要素 | 位置 | スタイル |
|---|---|---|
| `OUT` ラベル | (206, 162) | ui-sans 8 regular, letter-spacing=2, color: text.secondary |
| プロトコル名 | (238, 162) | ui-sans 9 medium, color: text.primary（"BLE" or "USB"） |
| プロファイル番号 | (266, 162) 右揃え | ui-sans 11 medium, color: accent |

区切り線 → OUT の間隔 = 10px（sep y=142、OUT y=162）。

### 5.7 KEYS（右パネル最下段）

| 要素 | 位置 | スタイル |
|---|---|---|
| `KEYS` ラベル | (206, 184) | ui-sans 8 regular, letter-spacing=2, color: text.secondary |
| カウント値 | (266, 184) 右揃え | ui-sans 11 medium, color: text.primary |

OUT → KEYS 間隔 = 22px（OUT y=162、KEYS y=184）。
...
├── header_dongle_icon (lv_obj custom drawn)
├── header_dongle_label (lv_label "D")
├── header_dongle_value (lv_label "96")
├── ring_track_p1 (lv_arc, range 0-100, value=100, track color)
├── ring_track_p2 (lv_arc)          ← 2ペリフェラル以上
├── ring_track_p3 (lv_arc)          ← 3ペリフェラルのみ
├── ring_p1 (lv_arc, value=batt_p1_pct, color=ring.p1)
├── ring_p2 (lv_arc, color=ring.p2) ← 2ペリフェラル以上
├── ring_p3 (lv_arc, color=ring.p3) ← 3ペリフェラルのみ
├── layer_name (lv_label "Base", Cormorant Garamond 36px)
├── batt_p1_dot (lv_obj circle, fill=ring.p1)
├── batt_p1_value (lv_label "85")
├── batt_p2_dot, batt_p2_value      ← 2ペリフェラル以上
├── batt_p3_dot, batt_p3_value      ← 3ペリフェラルのみ
├── divider (lv_obj 縦線)
├── mod_c (lv_obj circle r=12 + lv_label "C")   ← CTRL
...
├── sep_line (lv_obj 横線)
├── out_label (lv_label "OUT")
├── out_proto  (lv_label "BLE"/"USB")
├── out_profile (lv_label "1"/"2"... or hidden)
├── keys_label (lv_label "KEYS")
└── keys_value (lv_label "8,247")
```

MOD/STATE の各 circle チップは `lv_obj` にカスタム描画（`lv_draw_arc`）か、`lv_arc` を全周 value=100 でトラックなしにして背景塗りとして使う方法で実装。ON/OFF の切り替えは circle の bg_color と label の color を変更するだけ。
...
| 2026-05-03 | MOD/STATEチップを矩形から丸型アイコンに変更（C/S/A/G/⇪/あ）。セクションラベル（LAYER/MODIFIERS/STATE）を削除。レイヤー名フォントをCormorant Garamondに確定。全レイアウトパラメータを確定 |
