...
int zmk_widget_output_info_init(struct zmk_widget_output_info *widget, lv_obj_t *parent) {
    ...
    lv_obj_set_pos(widget->dongle_group, 218, 8);
    ...
    lv_obj_set_style_text_font(dongle_label, &DINishCondensed_SemiBold_20, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(out_header, &DINishCondensed_SemiBold_20, LV_PART_MAIN);
    lv_obj_set_style_text_color(out_header, lv_color_hex(RING_COLOR_TEXT_TER), LV_PART_MAIN);
    lv_obj_set_style_text_letter_space(out_header, 1, LV_PART_MAIN);
    ...
    lv_obj_set_pos(widget->out_proto, 238, 172);
    ...
    lv_obj_set_pos(widget->out_profile, 264, 172);
    lv_obj_add_flag(widget->out_profile, LV_OBJ_FLAG_HIDDEN);

    // ── KEYS section ─────────────────────────────────────────
    lv_obj_t *keys_header = lv_label_create(parent);
    lv_label_set_text(keys_header, "KEYS");
    lv_obj_set_style_text_font(keys_header, &DINishCondensed_SemiBold_20, LV_PART_MAIN);
    lv_obj_set_style_text_color(keys_header, lv_color_hex(RING_COLOR_TEXT_TER), LV_PART_MAIN);
    lv_obj_set_style_text_letter_space(keys_header, 1, LV_PART_MAIN);
    ...
    lv_obj_set_pos(widget->keys_value, 238, 200);
